...
export function MonthlyTable({ output, name: _name, fxRate }: { output: CompOutput; name: string; fxRate: number }) {
        ...
        <table className="w-full min-w-[700px]">
          <thead>
            <tr className="border-b border-border">
              {['Month', 'Rate', 'Base', 'Commission', 'Gross', 'Deductions', 'Net USD', 'Net CAD'].map((h) => (
                ...
                <Section key={gi}>
                  ...
                  {g.months.map((m) => (
                    <tr key={m.month} className="border-b border-border-subtle/50 transition-colors hover:bg-surface2">
                      ...
                      <td className="px-4 py-3 pr-5 text-sm tabular-nums text-muted">{cad(m.netCAD)}</td>
                    </tr>
                  ))}
                </Section>
              );
          ...
          <tfoot>
            <tr className="border-t border-border bg-surface2/30">
              ...
              <td className="px-4 py-3.5 pr-5 text-sm tabular-nums font-bold text-muted">{cad(output.netTotalCAD)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
